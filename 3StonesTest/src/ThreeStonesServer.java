 ...
 * @author derek
 */
public class ThreeStonesServer implements Runnable  {
    ...
    public ThreeStonesServer(InputStream in, OutputStream out, Socket clntSock) throws IOException{
        game = new Game();
//        int servPort = 50000;
...
//        System.out.println("Server initialized. IP: " + InetAddress.getLocalHost().getHostAddress() + ", PORT: " + servPort);
        ...
        for (;;) {
            ...
            try {
//                game.reset(); 
//                new Thread(new ThreeStonesServer().start();
            int servPort = 50000;
            ServerSocket servSock = new ServerSocket(servPort);
                Socket clntSock = servSock.accept();	// Get client connection
//
...
//
                InputStream in = clntSock.getInputStream();
                OutputStream out = clntSock.getOutputStream();
                new Thread(new ThreeStonesServer(in, out, clntSock)).start();
//
//                while ((recvMsgSize = in.read(byteBuffer)) != -1) { //This represents a game
//                    int recvInts[] = convertBytesToIntArrays(byteBuffer);
//
